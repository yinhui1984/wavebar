...
### 2. 悬浮控制栏参数详解
当鼠标悬停在 Wavebar 窗口内时，底部的半透明磨砂控制栏会自动淡入。它提供了以下 9 个精准控制器：
...
| **LIQUID FX** | 勾选框 | **GPU 液态凝胶渲染模式**。开启时，Metal GPU 着色器对频谱画布执行实时后处理：相邻频谱柱在靠近时会通过 Alpha 阈值平滑融合"液体化"在一起（Metaball 元球效果），同时辉光随低频鼓点强度动态脉动。全程 GPU 硬件加速，零额外 CPU 开销。 |
