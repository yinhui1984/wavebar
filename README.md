...
鼠标悬停在 Wavebar 窗口内时，半透明磨砂控制栏会自动淡入。常用调节参数定义如下：
...
| **VISUALIZER STYLE** | 下拉菜单 | 切换可视化效果风格：<br>• `Frequency Bars` (柱状频谱)<br>• `3D Particle Flow` (3D 粒子流) |
...
| **PARTICLE SPEED** | 滑块 | 粒子流动速度倍率（`0.3x` ~ `2.0x`，**仅在 3D Particle Flow 风格下显示**）。 |
| **TURBULENCE STRENGTH** | 滑块 | 湍流风场抖动强度（`0.2x` ~ `2.5x`，**仅在 3D Particle Flow 风格下显示**）。 |
| **VORTEX SIZE** | 滑块 | 漩涡环绕半径缩放因子（`0.5x` ~ `2.0x`，**仅在 3D Particle Flow 风格下显示**）。 |
...
| **BASS LIMIT** | 滑块 | 频谱分析低音下限（`20Hz` ~ `1000Hz`），可与鼠标水平拖拽/平移手势联动。 |
| **TREBLE LIMIT** | 滑块 | 频谱分析高音上限（`1000Hz` ~ `22000Hz`），可与鼠标水平拖拽/平移手势联动。 |

### 2. 自适应折叠布局
- **控制栏折叠**：当窗口高度小于 `180` px 或宽度小于 `360` px 时，底部的悬浮控制栏自动淡出隐藏，腾出全部画布空间用于显示可视化图形。
- **手势保留**：控制栏折叠后，画布上的 **水平拖拽缩放与平移手势** 仍保持激活状态，可通过拖拽调节分析频段。
...
* **原理**：
  ...
  $$F = -k \cdot x - c \cdot v$$
  其中刚度系数 $k=0.16$，阻尼比 $c=0.14$。当重低音脉冲判定检测到超强鼓点冲击时，向下的瞬时初始速度脉冲 `shakeVelocity = glow * 3.5` 注入该系统。使得整个频谱 Canvas 会像物理喇叭盆体一样产生下沉与阻尼弹回的颤震物理位移，增加了画面的实体冲击感。
